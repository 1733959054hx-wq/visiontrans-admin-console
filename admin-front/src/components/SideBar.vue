...
<template>
  <aside
    ...
    class="sidebar-bg relative z-20 flex w-[248px] flex-none flex-col overflow-hidden text-white"
  >
    <!-- 品牌区 -->
    <div class="relative z-10 flex items-center gap-3 px-5 py-5">
      <div
        ...
        style="background: linear-gradient(135deg, #2563eb, #0ea5e9); box-shadow: 0 8px 20px -8px #0ea5e9"
      >
        <i class="fa-solid fa-vr-cardboard"></i>
      </div>
      <div class="leading-tight">
        <div class="text-[15px] font-bold tracking-wide">{{ app.system?.name || '视界译 VisionTrans' }}</div>
        <div class="text-[10.5px] tracking-wider text-slate-300/80">{{ app.system?.subtitle || '' }}</div>
      </div>
    </div>

    <!-- 集群状态 -->
    <div class="relative z-10 px-5 pb-3">
      <div class="glass-dark flex items-center gap-2.5 rounded-xl px-3 py-2.5">
        <span class="dot-live h-2 w-2 rounded-full bg-emerald-400"></span>
        <div class="text-[11.5px] leading-tight">
          <div class="font-semibold">{{ app.system?.clusterStatus || '集群正常' }}</div>
          <div class="text-[10px] text-slate-300/70">{{ app.system?.clusterLatency || '' }}</div>
        </div>
      </div>
    </div>
    ...
    <!-- 导航 -->
    <div
      class="relative z-10 px-4 pb-1 pt-3 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400/70"
    >
      {{ app.nav.group }}
    </div>
    ...
    <!-- 当前管理员 -->
    <div class="relative z-10 border-t border-white/10 px-4 py-4">
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0 flex-1 leading-tight">
          ...
          <div class="truncate text-[10px] text-slate-300/70">ID: {{ app.system?.currentUserId || '—' }}</div>
        </div>
        <i
          class="fa-solid fa-ellipsis cursor-pointer text-xs text-slate-300/70 transition hover:text-white"
          title="折叠侧栏"
