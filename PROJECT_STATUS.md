...
14. **联调纪律（商户负责人定的工作规矩，每次会话必须遵守）**：① 联调/演示启动的前后端服务，**跑完必须停掉归还**（8080/5173 不能留后台任务）；② 联调产生的**测试临时文件用完即删**（uploads 下测试文件、%TEMP% 下的临时脚本，不留到下次会话）；③ **代码每更新一次就 commit + push 一次**（origin + upstream 都推），不留未推送的本地提交。
